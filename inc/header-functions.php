    ...
    function ample_header_title()
{
        if (is_archive()) {
            ...
            elseif (is_author()):
                 ...
                 */
                the_post();
                $ample_header_title = sprintf(__('Auteur: %s', ''), '<span class="vcard">' . get_the_author() . '</span>');
            ...
            elseif (is_day()):
                $ample_header_title = sprintf(__('Jour: %s', ''), '<span>' . get_the_date() . '</span>');

            elseif (is_month()):
                $ample_header_title = sprintf(__('Mois: %s', ''), '<span>' . get_the_date('F Y') . '</span>');

            elseif (is_year()):
                $ample_header_title = sprintf(__('Année: %s', ''), '<span>' . get_the_date('Y') . '</span>');

            elseif (is_plugin_active('woocommerce/woocommerce.php')):
                $ample_header_title = woocommerce_page_title(false);

            else:
                $ample_header_title = __('Archives', '');

            endif;
        } elseif (is_404()) {
        $ample_header_title = __('Page non trouvée', '');
    } elseif (is_search()) {
        $ample_header_title = sprintf(__('Résulat de recherche pour : %s', 'ample'), '<span>' . get_search_query() . '</span>');
    } elseif (is_page()) {
    ...
    function ample_breadcrumb()
{
        if (function_exists('bcn_display')) {
            ...
            echo '<span class="breadcrumb-title">' . __('Vous êtes ici :', '') . '</span>';
            bcn_display();
            echo '</div> <!-- .breadcrumb -->';
        }
    }
